var WeatherNowExtension  = (function(){
    ...
    WeatherNow.Models.Weather = Backbone.Model.extend({
        ...
        parse: function(data){
            ...
            return _.extend(data, {user_friendly_temp:{
    ...
    WeatherNow.Views.WeatherWidgetViewLeft = Backbone.View.extend({
        ...
        render: function() {
            this.$el.html(Mustache.render(this.template, _.extend(this.model.attributes,{
                f : WeatherNow.UserSettings.cache && WeatherNow.UserSettings.cache.scale === "c" ? false : true
            }))).removeClass("hide");
        }
    });
    ...
    WeatherNow.Views.WeatherWidgetViewLeftSettings = Backbone.View.extend({
        ...
        closeSettings: function(){
            ...
            WeatherNow.UserSettings.set({
                city: city_name,
                scale: WeatherNow.UserSettings.cache ? WeatherNow.UserSettings.cache.scale : "f"
            }, function(err, sett){
    ...
    WeatherNow.UserSettings = {
        get: function(cb){
            ...
            if(WeatherNow.UserSettings.cache){
                return WeatherNow.UserSettings.cache;
            }else{
                chrome.storage.local.get("WeatherWidgetSettings", function(err, settings){
                    if(settings){
        ...
        set:function(settings, callback){
            var _sett = _.extend({}, _.pick(settings, "city", "scale"));
    ...
    var init = function(){
        ...
        new WeatherNow.Views.WeatherWidgetViewLeft({model:weatherWidget});
        new WeatherNow.Views.WeatherWidgetViewLeftSettings({model:weatherWidget});
        new WeatherNow.Views.WeatherWidgetViewTopRight({model:weatherWidget});
        new WeatherNow.Views.WeatherWidgetViewBottomRight({model:weatherWidget});
        ...
        async.waterfall([
            ...
            weatherWidget.fetch({
                data: {q:result.user_settings.city}
            });
        });
